...
async function cc_call(fn_name, args){
    ...
    if(fn_name == 'createRecordCopy')
    {   patno=args[0];
        patname=args[1];
        recordhash=args[2];
       result = await contract.submitTransaction('createRecordCopy', patno, patname, recordhash); 
    }
    else if( fn_name == 'VerificationTicket')
        result = await contract.evaluateTransaction('VerificationTicket', VerificationTicket);
    else if(fn_name == 'queryPatNo')
...
// find VerificationTicket
app.post('/findVerificationTicket/:VerificationTicket', async (req,res)=>{
    const VerificationTicket = req.body.VerificationTicket;
    console.log("TicketNumber: " + req.body.Veri1ficationTicket);
    console.log("TicketNumber_typeof: " + typeof(VerificationTicket));
    const walletPath = path.join(process.cwd(), 'wallet');
    const wallet = new FileSystemWallet(walletPath);
